...
/**
 * Refreshes one character in the local list without scanning the full library.
 * @param {string} avatarUrl Character avatar filename
 * @returns {Promise<object|undefined>} The refreshed character, if available
 */
 ...
 * @param {boolean} [options.simple=false] Return filenames without parsing chat metadata
 *
...
async function importCharactersTags(avatarFileNames) {
    // Refresh only imported cards. A full getCharacters() scan also walks every
    // character's chat directory, which makes large libraries slow after import.
    await Promise.all(avatarFileNames.map(avatarFileName => getOneCharacter(avatarFileName)));
...
export async function deleteCharacter(characterKey, { deleteChats = true } = {}) {
    ...
    for (const key of characterKey) {
        ...
        const pastChats = deleteChats ? await getPastCharacterChats(chid, { simple: true }) : [];
    ...
    await removeCharacterFromUI(characterKey);
    return deleted;
}
 ...
 * panel, removing character name from navigation tabs, clearing chat, and updating the list incrementally.
 * It also ensures to save the settings after all the operations.
 */
async function removeCharacterFromUI(deletedCharacterKeys = []) {
    preserveNeutralChat();
    await clearChat();
    ...
    const deletedKeys = new Set(Array.isArray(deletedCharacterKeys) ? deletedCharacterKeys : [deletedCharacterKeys]);
    for (let i = characters.length - 1; i >= 0; i--) {
        if (deletedKeys.has(characters[i].avatar)) {
            characters.splice(i, 1);
        }
    }

    // Groups may reference deleted characters, but the character list itself
    // no longer needs a full server-side scan after a successful deletion.
    await getGroups();
    await printCharacters(true);
    await printMessages();
    saveSettingsDebounced();
