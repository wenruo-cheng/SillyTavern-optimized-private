...
import { getChatDirectoryStats, invalidateChatDirectoryStats } from '../chat-directory-stats.js';
    ...
    async verify(directoriesList) {
        try {
            ...
            for (const dir of directoriesList) {
                ...
                const pngFiles = files.filter(file => file.isFile() && path.extname(file.name) === '.png');
                const verifyConcurrency = 32;
                for (let i = 0; i < pngFiles.length; i += verifyConcurrency) {
                    const keys = await Promise.all(pngFiles.slice(i, i + verifyConcurrency).map(async file => {
                        const filePath = path.join(dir.characters, file.name);
                        const cacheKey = await getCacheKey(filePath);
                        return path.parse(cache.getDatumPath(cacheKey)).base;
                    }));
                    keys.forEach(key => validKeys.add(key));
                }
            }
            const hashedKeys = await fs.promises.readdir(this.cachePath);
    ...
    try {
        ...
        async function getInputImage() {
            try {
                ...
                // Metadata-only PNG writes do not need a Jimp decode/re-encode
                // cycle. Preserve the original pixels and let the PNG writer
                // update only the character metadata chunks.
                if (!crop && path.extname(inputFile).toLowerCase() === '.png') {
...
const processCharacter = async (item, directories, { shallow }) => {
    try {
        ...
        const charStat = await fs.promises.stat(path.join(directories.characters, item));
        ...
        const { chatSize, dateLastChat } = await getChatDirectoryStats(chatsDirectory);
...
router.post('/delete', validateAvatarUrlMiddleware, async function (request, response) {
    ...
    if (request.body.delete_chats == true) {
        ...
        invalidateChatDirectoryStats(chatsPath);
        try {
            if (fs.existsSync(chatsPath)) {
                const pendingDeletePath = path.join(request.user.directories.chats, `.deleting-${sanitize(dir_name)}-${Date.now()}`);
                await fs.promises.rename(chatsPath, pendingDeletePath);
                setImmediate(() => {
                    fs.promises.rm(pendingDeletePath, { recursive: true, force: true })
                        .catch(error => console.error(`Failed to remove chats for ${dir_name}:`, error));
                });
            }
        } catch (error) {
            console.error(`Failed to prepare chat deletion for ${dir_name}:`, error);
            return response.sendStatus(500);
        }
    }
...
router.post('/all', async function (request, response) {
    try {
        ...
        const data = [];
        const processingConcurrency = 16;
        for (let i = 0; i < pngFiles.length; i += processingConcurrency) {
            const batch = pngFiles.slice(i, i + processingConcurrency);
            const characters = await Promise.all(batch.map(file => processCharacter(file, request.user.directories, { shallow: useShallowCharacters })));
            data.push(...characters.filter(character => character.name));
        }
...
router.post('/chats', validateAvatarUrlMiddleware, async function (request, response) {
    try {
        ...
        const files = await fs.promises.readdir(chatsDirectory, { withFileTypes: true });
