...
import { invalidateChatDirectoryStats } from '../chat-directory-stats.js';
import {
    getConfigValue,
    humanizedDateTime,
    tryParse,
...
export async function trySaveChat(chatData, filePath, skipIntegrityCheck = false, handle, cardName, backupDirectory) {
    ...
    invalidateChatDirectoryStats(path.dirname(filePath));
...
router.post('/rename', validateAvatarUrlMiddleware, async function (request, response) {
    try {
        ...
        invalidateChatDirectoryStats(pathToFolder);
...
router.post('/delete', validateAvatarUrlMiddleware, function (request, response) {
    try {
        ...
        if (tryDeleteFile(chatFilePath)) {
            invalidateChatDirectoryStats(path.dirname(chatFilePath));
            return response.send({ ok: true });
        } else {
...
router.post('/import', validateAvatarUrlMiddleware, function (request, response) {
    ...
    try {
        ...
        if (format === 'json') {
            ...
            invalidateChatDirectoryStats(directoryPath);
