...
let deferredStartupMaintenance = null;
app.use(helmet({
...
async function preSetupTasks() {
    ...
    // These tasks scan user data and are not required to bind the HTTP server.
    // Defer them until after the server is listening so the UI can become
    // available while maintenance continues in the background.
    deferredStartupMaintenance = async () => {
...
async function postSetupTasks(result) {
    if (deferredStartupMaintenance) {
        const maintenance = deferredStartupMaintenance;
        deferredStartupMaintenance = null;
        setImmediate(() => {
            maintenance().catch(error => console.error('Deferred startup maintenance failed:', error));
        });
    }
